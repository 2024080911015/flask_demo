...
def run_pipeline():
    ...
    T+1 离线重训流水线 (GCN 核心版)
    """
    print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] 启动 GNN 认知进化流水线...")
    env = os.environ.copy()
    ...
    try:
        ...
        s1 = subprocess.run([sys.executable, "step1_full_process.py"], capture_output=True, text=True, encoding="utf-8", errors="replace", env=env)
        if s1.returncode != 0: raise Exception(f"Step 1 失败: {s1.stderr}")

        # 2. 运行 Step 2: GCN 深度学习
        print(" [2/3] 正在训练 GCN 链路预测模型...")
        s2 = subprocess.run([sys.executable, "step2_train_full.py"], capture_output=True, text=True, encoding="utf-8", errors="replace", env=env)
